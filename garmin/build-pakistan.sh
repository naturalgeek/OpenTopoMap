...
#wget -c "https://download.geofabrik.de/${continent}/${countryname_short}-latest.osm.pbf"
...
java -Xmx5000m -jar $MKGMAP_JAR --output-dir=$MKGMAP_OUTPUT_DIR --keep-going --style-file=$MKGMAP_STYLE_FILE --description="Waldrian ${countryname_short} ${continentdate}" --area-name="Waldrian ${countryname_short} ${continentdate}" --overview-mapname="Waldrian_${countryname_short}" --family-name="Waldrian ${countryname_short} ${continentdate}" --family-id=$FAMILY_ID --series-name="Waldrian ${countryname_short} ${continentdate}" --bounds=$BOUNDS_FILE --precomp-sea=$SEA_FILE -c $MKGMAP_OPTS $REDUCED_DENSITY $GMAPI $mkgmapin $MKGMAP_TYP_FILE &> $MKGMAP_OUTPUT_DIR/mkgmap.log
...
mv output/gmapsupp.img output/Waldrian_${countryname_short}_garmin.img
#./src/jmc_cli -src=output -dest=output -bmap=${countryname_short}.img -gmap="pakistan.gmap"

#DATE=`date +%s`
#mkdir -p output/${countryname_short}

#mv output/${countryname_short}.img output/pakistan/pakistan_${DATE}.img
#mv output/${countryname_short}.gmap output/pakistan/pakistan_${DATE}.gmap
