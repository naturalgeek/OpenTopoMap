...
java -Xmx10000m -jar $MKGMAP_JAR --output-dir=$MKGMAP_OUTPUT_DIR --style-file=$MKGMAP_STYLE_FILE --description="Waldrian TOPO ${countryname_short^} ${continentdate}" --area-name="Waldrian TOPO ${countryname_short^} ${continentdate}" --overview-mapname="Waldrian_TOPO_${countryname_short^}" --family-name="Waldrian TOPO ${countryname_short^} ${continentdate}" --family-id=$FAMILY_ID --series-name="Waldrian TOPO ${countryname_short^} ${continentdate}" --bounds=$BOUNDS_FILE --precomp-sea=$SEA_FILE -c $MKGMAP_OPTS $REDUCED_DENSITY $GMAPI $mkgmapin $MKGMAP_TYP_FILE &> $MKGMAP_OUTPUT_DIR/mkgmap.log
